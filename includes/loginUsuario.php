	...
	if(empty($_POST['login']) || empty($_POST['pass'])) {
		...
		if ($query->rowCount() > 0) {
			if (password_verify($pass, $result['clave'])) {
				...
				echo '<div class="alert alert-success"><button type="button" class="close" date-dismiss="alert"></button>Redirecting</div>';
			} else{
				echo '<div class="alert alert-warning"><button type="button" class="close" date-dismiss="alert"></button>Usuario inactivo, comuniquese con el administrador</div>';
			}
			
			}else {
				echo '<div class="alert alert-danger"><button type="button" class="close" date-dismiss="alert"></button>Usuario o contraseña incorrectos</div>';
				}
			}else {
				echo '<div class="alert alert-danger"><button type="button" class="close" date-dismiss="alert"></button>Usuario o contraseña incorrectos</div>';
		}
